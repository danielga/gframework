...
function gframework.playerdata:SetPlayerData(ply, varname, value)
	...
	gframework.database:Query("UPDATE test SET PlayerData = " .. escapeText(gframework.playerdata:CompressData(ply.GFrameworkData.PlayerData)) .. " WHERE SteamID = " .. escapeText(ply:SteamID()))

	ply:SetNetworkedVar("GFWPlayerData_" .. varname, value)
end

function PLAYER:SetPlayerData(varname, value)
...
local function gframeworkPlayerInitialSpawn(ply)
	...
	gframework.database:Query("SELECT PlayerData FROM test WHERE SteamID = " .. escapeText(ply:SteamID()), function(data) databaseQueryComplete(data, ply) end, databaseQueryFailed)
end
